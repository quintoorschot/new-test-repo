    ...
    if test_info:
        ...
        while round <= 5:
            ...
            if test_result == "test_pass":
                ...
                good_patch_dir = os.path.join(root_path, "good_patches")
                os.makedirs(good_patch_dir, exist_ok=True)

                # Use full test name, replace dots with underscores to make a safe filename
                test_name_safe = test.replace('.', '#')
                out_path = os.path.join(good_patch_dir, f"good_patch.{test_name_safe}.java")

                with open(out_path, "w") as f:
                    f.write(test_class_content)

                print(f"[final] saved {out_path}", flush=True)
